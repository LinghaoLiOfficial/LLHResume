...
- 移动端正文当前采用 CSS 统一控制标题降级、正文行高提升、长文本 `overflow-wrap` 和最多 5 行截断；`src/views/resume/Resume.vue` 会在 640px 以下检测实际被截断的长段落，并把可展开状态提升到对应 `article` 条目上，支持点击整张条目或键盘 Enter/空格展开/收起。
